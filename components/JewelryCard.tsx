...
import { getShortDescription, type Jewelry } from "@/lib/joyeria";

export default function JewelryCard({ item }: { item: Jewelry }) {
  return (
    <Link href={`/joyeria/${item.id}`} className="group block">
      <motion.div
      ...
      <div className="text-center px-2">
        ...
        <div className="font-sans font-semibold text-[16px] tracking-[0.04em]">{item.precio_mxn}</div>
      </div>
    </Link>
  );
}
